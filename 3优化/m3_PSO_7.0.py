...
class PSO:
    def __init__(self, pN, dim, max_iter, bd_low, bd_up, v_low, v_up):  # 初始化PSO参数
        ...
        self.fit = -10086                                # 全局最佳适应值  让R2_score做适应值，越高越好
    ...
    def init_Population(self):  # 初始化粒子群
        for i in range(self.pN):
            for j in range(self.dim):
                ...
                self.pbest[i][j] = self.pos[i][j]
                kk.input_settings(dd, self.pos[i][1], 1, self.pos[i][0], self.pos[i][2], self.pos[i][3])
                ppp = kk.do_svr(x_train, y_train)
                self.p_fit[i] = ppp.score(x_test, y_test)
                if self.fit <= self.p_fit[i]:
    ...
    def iterator(self):
        ...
        for t in range(self.max_iter):
            ...
            for i in range(self.pN):
                ...
                if float( temp_r2_test ) > float(self.p_fit[i]) and temp_r2_train < 0.96:    # 更新个体最优 and 防止过拟合
                # if float( temp_r2_test ) > float(self.p_fit[i]):    # 更新个体最优 and 防止过拟合
                ...
                # print( dd, self.pos[i][1], self.pos[i][0], self.pos[i][2], self.pos[i][3] )
