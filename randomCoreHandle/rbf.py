        ...
        for t in range(self.max_iter):
            ...
            for i in range(self.pN):
                ...
                self.v[i] = (self.w * self.v[i] +
                             self.c1 * self.r1 * (self.pbest[i] - self.v[i]) +
                             self.c2 * self.r2 * (self.gbest    - self.v[i]) )
            ...
            # 设置混合核函数 查看
            # 设置每次迭代的最优参数
            kk.set_cc(self.gbest[0])
            kk.set_gamma(self.gbest[0])
            svr = kk.do_svr(x_train, y_train)
            ...
            print('最佳参数为  cc gg', )
            print( self.gbest[0], ",", self.gbest[1])
        return fitness
...
if __name__ == '__main__':
    ...
    my_pso = PSO(pN=50, dim=2, max_iter=iter1, bd_low=bd_low1, bd_up=bd_up1, v_low=v_low1, v_up=v_up1)
