...
class PSO:
    ...
    def init_Population(self, bd_low, bd_up, v_low, v_up):
        for i in range(self.pN):
            ...
            temp_r2_test, temp_r2_train = self.function(self.X[i])     # 返回r2
            self.p_fit[i] = temp_r2_test
            if temp_r2_test > self.fit and temp_r2_train < guoNiHe:
                self.fit = temp_r2_test
                self.gbest = self.X[i]
    ...
    def iterator(self, bd_low, bd_up, v_low, v_up):
        ...
        for t in range(self.max_iter):
            ...
            for i in range(self.pN):                    # 更新gbest 和 pbest
                temp_r2_test, temp_r2_train = self.function(self.X[i])         # 获取当前粒子对应参数的 score ，即 训练集R^2
                if float(temp_r2_test) > float(self.p_fit[i]) and (temp_r2_train < guoNiHe):  # 更新个体最优
                    self.p_fit[i] = temp_r2_test
                    self.pbest[i] = self.X[i]           # 更新个体最优对应的的矩阵
                if (self.p_fit[i] > self.fit) and (temp_r2_train < guoNiHe):            # 更新全局最优
...
if __name__ == '__main__':
    #             C,     q,     t,  gamma, coef0
    bd_low1 = [   70,   0.2,    0,      0,    70  ]
    bd_up1  = [   120,   1,      0.5,   40,   220 ]
    v_low1  = [   0,     0,      0,     0,     0  ]
    v_up1   = [   25,   0.5,   0.5,     20,   70  ]
